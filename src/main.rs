...
fn random_chance() -> (bool, u8) {
    ...
    let imposter = rng.gen_range(1..22);
...
fn remove_imposters(path: String) -> u32 {
    ...
    for (index, character) in old_data.as_bytes().iter().enumerate() {
        if old_data.len() > index + 4 && index > 4 {
            if old_data.as_bytes()[index + 1] == 47
                && old_data.as_bytes()[index + 2] == 47
                && old_data.as_bytes()[index + 3] == 46
                && old_data.as_bytes()[index + 4] == 63
            {
                imposters += 1;
                deleting = true;
            }
            ...
            if old_data.as_bytes()[index - 1] == 47
                && old_data.as_bytes()[index - 2] == 47
                && old_data.as_bytes()[index - 3] == 63
                && old_data.as_bytes()[index - 4] == 46
            {
                deleting = false;
            }
        }
